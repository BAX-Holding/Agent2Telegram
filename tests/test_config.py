...
class ConfigTests(unittest.TestCase):
    ...
    def test_stt_language_code_is_trimmed_and_may_be_empty(self):
        cfg = Config(agent="codex", token="1:2", allowed_user_ids=[1],
                     stt_language_code="  sk  ")
        cfg.validate()
        self.assertEqual(cfg.stt_language_code, "sk")

        cfg.stt_language_code = "   "
        cfg.validate()
        self.assertEqual(cfg.stt_language_code, "")

    def test_stt_language_code_must_be_a_string(self):
        with self.assertRaises(ConfigError):
            Config(agent="codex", token="1:2", allowed_user_ids=[1],
                   stt_language_code=123).validate()
