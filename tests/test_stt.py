...
class STTTests(unittest.TestCase):
    ...
    def test_empty_language_code_uses_provider_auto_detection(self):
        op = _FakeOpener({"text": "ahoj"})
        stt.transcribe_elevenlabs(b"audio", api_key="k", language_code="", opener=op)
        self.assertNotIn(b'name="language_code"', op.last_request.data)
